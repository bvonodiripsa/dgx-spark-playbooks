...
// Generate cluster colors using a golden ratio approach for good distribution
const generateClusterColors = (numClusters: number): string[] => {
  ...
  for (let i = 0; i < numClusters; i++) {
    ...
    // Use high saturation and good brightness for visibility
    const saturation = 70 + (i % 3) * 10; // 70-90%
    const lightness = 50 + (i % 4) * 10;  // 50-80%
          ...
          if (debugInfo.includes("cluster-viz")) {
            ...
            try {
              ...
              clusters.forEach((nodes, clusterIndex) => {
                // Generate a color based on cluster index
                const hue = (clusterIndex * 137) % 360; // Golden ratio to spread colors
                const color = `hsl(${hue}, 70%, 50%)`; // Slightly less saturated to avoid WebGL issues
    ...
    <div className="relative w-full h-full overflow-hidden bg-gray-900">
      ...
      <div className="absolute top-4 right-24 z-10 bg-gray-800/80 p-3 rounded text-xs text-gray-300 shadow w-48">
        ...
        <p className="mt-1"><span className="font-semibold text-white">WebGPU Clustering:</span> 
          <span className={isClusteringEnabled && !usingCpuFallback ? 'text-green-400' : 'text-gray-400'}>
            {isClusteringAvailable ? (
              isClusteringEnabled ? (
                usingCpuFallback ? 'Enabled (CPU Fallback)' : 
                clusteringEngineRef.current?.isUsingRemote() ? 'Enabled (Remote GPU)' : 'Enabled (Local GPU)'
              ) : 'Disabled'
            ) : 'Not Available'}
          </span>
        </p>
