...
export function SettingsModal() {
  ...
  const [jenaEndpoint, setJenaEndpoint] = useState("http://localhost:3030")
  const [jenaDataset, setJenaDataset] = useState("txt2kg")
  const [jenaUser, setJenaUser] = useState("")
  const [jenaPassword, setJenaPassword] = useState("")
  ...
  useEffect(() => {
    ...
    // Load Jena settings
    setJenaEndpoint(localStorage.getItem("jena_endpoint") || "http://localhost:3030")
    setJenaDataset(localStorage.getItem("jena_dataset") || "txt2kg")
    setJenaUser(localStorage.getItem("jena_user") || "")
    setJenaPassword(localStorage.getItem("jena_password") || "")
  ...
  const saveDbSettings = async (e: React.FormEvent) => {
    ...
    // Save Jena settings
    localStorage.setItem("jena_endpoint", jenaEndpoint)
    localStorage.setItem("jena_dataset", jenaDataset)
    localStorage.setItem("jena_user", jenaUser)
    localStorage.setItem("jena_password", jenaPassword)
      ...
      await fetch('/api/settings', {
        ...
        body: JSON.stringify({
          settings: {
            ...
            jena_endpoint: jenaEndpoint,
            jena_dataset: jenaDataset,
            jena_user: jenaUser,
            jena_password: jenaPassword
          }
        }),
      });
    } catch (error) {
    ...
    toast.success("Graph database settings saved");
    setIsOpen(false)
  }
  ...
  const saveS3Settings = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.success(`Connected to S3 bucket. Found ${files.length} files.`)
    } catch (error) {
      ...
      toast.error("S3 connection failed: " + (error instanceof Error ? error.message : "Unknown error"))
    } finally {
  ...
  const saveOllamaSettings = (e: React.FormEvent) => {
    ...
    toast.success("Ollama model settings saved")
  }
              ...
              <form onSubmit={saveDbSettings} className="space-y-4">
                <div className="space-y-2">
                  ...
                  >
                    ...
                    <option value="jena">Apache Jena Fuseki</option>
                  </select>
                </div>
                ...
                {graphDbType === "jena" && (
                  <div className="bg-background/50 rounded-lg p-3 space-y-3">
                    <h4 className="text-sm font-medium text-foreground mb-2">Apache Jena Fuseki Configuration</h4>
                    <div className="grid grid-cols-1 gap-3">
                      <div className="grid grid-cols-2 gap-3">
                        <div>
                          <label className="text-xs font-medium text-muted-foreground mb-1 block">Fuseki Endpoint</label>
                          <input
                            type="text"
                            value={jenaEndpoint}
                            onChange={(e) => setJenaEndpoint(e.target.value)}
                            placeholder="http://localhost:3030"
                            className="w-full bg-background border border-border/60 rounded-md p-2 text-sm text-foreground focus:ring-1 focus:ring-primary/50 focus:border-primary transition-colors"
                          />
                        </div>
                        <div>
                          <label className="text-xs font-medium text-muted-foreground mb-1 block">Dataset Name</label>
                          <input
                            type="text"
                            value={jenaDataset}
                            onChange={(e) => setJenaDataset(e.target.value)}
                            placeholder="txt2kg"
                            className="w-full bg-background border border-border/60 rounded-md p-2 text-sm text-foreground focus:ring-1 focus:ring-primary/50 focus:border-primary transition-colors"
                          />
                        </div>
                      ...
                      <div className="grid grid-cols-2 gap-3">
                        <div>
                          <label className="text-xs font-medium text-muted-foreground mb-1 block">Username (Optional)</label>
                          <input
                            type="text"
                            value={jenaUser}
                            onChange={(e) => setJenaUser(e.target.value)}
                            placeholder="admin"
                            className="w-full bg-background border border-border/60 rounded-md p-2 text-sm text-foreground focus:ring-1 focus:ring-primary/50 focus:border-primary transition-colors"
                          />
                        </div>
                        <div>
                          <label className="text-xs font-medium text-muted-foreground mb-1 block">Password (Optional)</label>
                          <div className="relative">
                            <input
                              type={showPassword ? "text" : "password"}
                              value={jenaPassword}
                              onChange={(e) => setJenaPassword(e.target.value)}
                              placeholder="admin"
                              className="w-full bg-background border border-border/60 rounded-md p-2 pr-8 text-sm text-foreground focus:ring-1 focus:ring-primary/50 focus:border-primary transition-colors"
                            />
                            <button
